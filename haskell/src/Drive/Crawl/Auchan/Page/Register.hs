...
module Drive.Crawl.Auchan.Page.Register (UserData(..), makeUserData, register) where
...
randomPhone :: IO Text
randomPhone = do
  phone <- mapM genDigit ([0..8] :: [Integer])
  return $ "08" <> foldl (\x y -> x <> show y) "" phone
    where genDigit _ = randomRIO (1, 10) :: IO Integer
...
makeUserData = do
  ...
  return UserData 
    { email1 = email
    , email2 = email
    , motDePasse1 = "Toto2222"
    , motDePasse2 = "Toto2222"
    , civilite = "MR"
    , organisme = "2"
    , raisonSociale = ""
    , nom = n
...
register u = do
  $(logDebug) formData
  _ <- load
  _ <- requestTag $ Req url "POST" hdrs formData
  return ()
    where 
