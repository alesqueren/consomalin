...
newtype CancelMessage = CancelMessage { transactionId :: Text }
  deriving (Typeable, Show, Eq, Generic)
instance FromJSON CancelMessage
instance ToJSON CancelMessage

data RabbitmqResource = RegistrationResource | CancelResource

getQueueName :: RabbitmqResource -> Text
getQueueName RegistrationResource = "registrations"
getQueueName CancelResource = "cancelTransactions"
...
send :: (ToJSON message) => RabbitmqResource -> message -> IO ()
send r m = do
  h <- fromEnvOr "AMQP_HOST" A.takeText "127.0.0.1"
  u <- fromEnvOr "AMQP_USER" A.takeText "guest"
  pass <- fromEnvOr "AMQP_PASS" A.takeText "guest"

  conn <- openConnection (T.unpack h) "/" u pass
  chan <- openChannel conn

  _ <- declareQueue chan newQueue {queueName = qname}
  _ <- publishMsg chan "" qname (newMsg {msgBody = encode m})

  return ()
    where qname = getQueueName r
