...
isTodayScaper :: EntityScraper Bool
isTodayScaper = EntityScraper
  { rootSelector = "a" @: ["href" @= "/drive/choixslot.retraitslotgrid.changejour/0"]
  , elementSelectors = M.fromList [ ("nameOfDay", text "a") ]
  , entityMaker = \m -> do
    n <- M.lookup "nameOfDay" m
    if n == " Aujourd'hui "
       then Just True
       else Nothing
  }
...
isToday :: BL.ByteString -> Bool
isToday p =
  case decode' p of
    Nothing -> False
    Just ds ->
      length (catMaybes $ entityScrap isTodayScaper $ parseTags $ zoneSlotGrid $ zones ds) > 0
...
getSchedule = do
  ...
  let interval =
        if isToday page
           then [0..(daysInSchedule-1)]
           else [1..daysInSchedule]

  slotsByDay <- mapM (\d -> extractFromJson (extractSlotInfo d) $ loadDay d) interval
