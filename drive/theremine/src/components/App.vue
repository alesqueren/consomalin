<template lang="pug">
  #app(@click='finishAllActions', @keyup.esc="finishAllActions", tabindex="0")
    ...
    div#content(v-bind:class="{'marginalize-left' : listRequired, 'marginalize-right' : navCarRequired}")
      div#replay
      div#flash(v-if="flashExist", v-bind:class="flashClasses") {{ flashMessage }}
      router-view
...
export default {
  ...
  computed: {
    ...
    flashMessage() {
      let message = '';
      if (this.flashExist) {
        if (this.flash.status === 'unknown') {
          message = `Une erreur est survenue, veuillez nous en excuser, renouvelez l'opération
          et contactez-nous si le problème persiste`;
        } else if (this.flash.status === 'slotUnavailable') {
          message = 'L\'horaire choisi n\'est plus disponible, veuillez en selectionner un autre';
        } else {
          message = 'Une erreur inconnue est survenu, veuilez nous contacter.';
        }
      }
      return message;
    },
